...
export default function Feed() {
              ...
              console.log(Object.keys(f_data_feed).length)
                ...
                for(let i=0; i<f_data_feed.length; i++)
                {
                  console.log(f_data_feed[i])
                    data_feed = data_feed+"<div class=' hover:cursor-pointer relative w-full flex flex-wrap bg-red-500 mt-2 mb-2 p-5 rounded-lg text-slate-900 bg-opacity-20' onClick='_setOpenPost(true)'><a class=' w-20 h-20 block rounded-full lg:ml-5 text-white bg-black'>ICON post by</a><p class=' lg:absolute h-24 top-5 w-64 text-left border-white left-32'><a class='font-bold block'>"+f_data_feed[i]['project_title']+"</a><span class=' text-xs block'>"+f_data_feed[i]['project_category']+"</span><span class=' text-xs block'>skills required - programming/WEB/ML/AI/database</span><span class=' text-xs inline'><FiUsers class='inline' />12</span></p><p class=' lg:absolute top-5 right-32 text-left overflow-auto scrollbar-hidden max-h-24 w-64'><span class='font-bold'>"+f_data_feed[i]['initiated_by']+"</span><span class=' text-xs block text-slate-600'>"+f_data_feed[i]['project_description']+"</span></p><p class='lg:absolute text-2xl hover:cursor-pointer hover:text-3xl top-12 right-5'></p></div>"
                }
                // console.log(data_feed)
                sessionStorage.setItem('data_feed', data_feed)
  ...
  return (
   <>
   {
    ...
    {
      // setInterval(() => {
        <div class=' min-w-full lg:flex justify-center'>
            <div id='feed' class=' lg:max-h-screen bg-red-300 scrollbar-hidden bg-opacity-20 overflow-auto w-full lg:w-1/2'>
